...
- The six approved event names, descriptions, derived categories, slugs, and detail links live in one typed `eventGalleryItems` collection. Clients, venues, dates, and photography remain unavailable, so every record preserves its stable identifier and explicit pending-media status without inventing asset paths.
...
- **Mosaïque Content Integration: Phase 7 complete.** The six approved names, categories, descriptions, slugs, detail destinations, status flags, and pending-media records remain centralized in `eventGalleryItems`; the Home gallery, archive, category routes, and detail pages consume that collection without parallel event data. Archive category routes derive from the source categories, visible rows remain limited to category and title, and each event now preserves the same placeholder tone across `All` and filtered views. No clients, venues, dates, locations, photography, or future destinations were invented.
