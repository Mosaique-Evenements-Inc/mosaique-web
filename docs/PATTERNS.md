...
Hero/About uses the default form. Hero occupies the `lead` slot as a full-viewport media scene; About is the solid panel that enters from the viewport edge and then continues as ordinary content. The Hero script measures progress from the non-sticky `PanelReveal` wrapper only to coordinate the navigation color state and media caption while the CSS handoff remains stable.
