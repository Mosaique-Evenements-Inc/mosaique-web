...
import type { LeadApiError, LeadFormInput, LeadSubmissionResult } from "../types/leads";
import { validateLeadPayload } from "../validators/leads.validators";
...
export const useLeadSubmission = () => {
  ...
  const submitLead = useCallback(
    async (input: LeadFormInput): Promise<LeadSubmissionResult> => {
      ...
      setSubmitting();
      try {
        const response = await createLead(validation.data);
